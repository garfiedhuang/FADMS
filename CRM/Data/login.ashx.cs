    ...
    public class login : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (request["Action"] == "login")
            {
                ...
                string loginType = PageValidate.InputText(request["login_type"], 10);
                ...
                if (!string.IsNullOrEmpty(validate) && !string.IsNullOrEmpty(username) && !string.IsNullOrEmpty(password))
                {
                    if (validate == context.Session["CheckCode"].ToString() || validate.ToLower() == context.Session["CheckCode"].ToString().ToLower())
                    {
                        ...
                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            if (ds.Tables[0].Rows[0]["canlogin"].ToString() == "1")
                            {
                                ...
                                if (loginType == "2")
                                {
                                    Garfield.BLL.Reports_FZ rFZ = new Garfield.BLL.Reports_FZ();
                                    var menus = rFZ.GetMenuTree(Convert.ToInt32(userid));
                                    if (string.IsNullOrEmpty(menus))
                                    {
                                        context.Response.Write(Common.DataToJson.SerializeObject(GetResponse(5, menus)));
                                    }
                                    else
                                    {
                                        context.Response.Write(Common.DataToJson.SerializeObject(GetResponse(2, menus)));
                                    }
                                }
                                else
                                {
                            ...
                            else
                            {
                                ...
                                if (loginType == "2")
                                {
                                    context.Response.Write(Common.DataToJson.SerializeObject(GetResponse(4, "")));
                                }
                                else
                                {
                        ...
                        else
                        {
                            ...
                            if (loginType == "2")
                            {
                                context.Response.Write(Common.DataToJson.SerializeObject(GetResponse(1, "")));
                            }
                            else
                            {
                    ...
                    else
                    {
                        ...
                        if (loginType == "2")
                        {
                            context.Response.Write(Common.DataToJson.SerializeObject(GetResponse(0, "")));
                        }
                        else
                        {
        ...
        private dynamic GetResponse(int status, string message)
        {
            return new
            {
                status = status,
